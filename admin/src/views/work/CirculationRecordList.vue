<template>
  <div class="page">
    <div class="page-header">
      <div><h1>作品流通记录</h1><p>管理作品从上架、收藏、保管、再次流通到证书关联的完整履历。</p></div>
      <div class="actions"><button class="ghost" @click="resetFilters">重置筛选</button><button class="primary" @click="refresh">刷新</button></div>
    </div>
    <div class="stats">
      <div class="stat"><label>有记录作品</label><b>{{ stats.s0 }}</b></div>
      <div class="stat"><label>再次流通中</label><b>{{ stats.s1 }}</b></div>
      <div class="stat"><label>平台保管</label><b>{{ stats.s2 }}</b></div>
    ...
    <div class="card">
      <div class="filters">
        ...
        <div class="field"><label>记录类型</label><select v-model="filters.status"><option value="">全部</option><option value="PENDING">待处理</option><option value="DONE">已完成</option></select></div>
    ...
    <div v-if="detailVisible" class="modal" @click.self="detailVisible=false">
      <div class="modal-box">
        <div class="modal-head"><div><h2>作品流通记录详情</h2><p>{{ current?.title }}</p></div><button class="text" style="color:white;font-size:28px" @click="detailVisible=false">×</button></div>
...
async function refresh(){
  ...
  try {
    ...
    list.value = data.filter(i => i.rawStatus === 'DONE' || i.rawStatus === 'PAID').map(item => ({
      id: item.id,
      title: (item.artworkTitle || '') + ' 流通记录',
    ...
    stats.s1 = data.filter(i => i.rawStatus === 'DONE').length
