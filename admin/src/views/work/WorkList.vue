<template>
  <div class="page">
    <div class="page-header">
      <div><h1>作品管理</h1><p>管理作品资料、图片、价格、状态、艺术家绑定、代表作、流通与保管配置。</p></div>
      <div class="actions"><button class="ghost" @click="resetFilters">重置筛选</button><button class="primary" @click="refresh">刷新</button></div>
    </div>
    <div class="stats">
      <div class="stat"><label>作品总数</label><b>{{ stats.s0 }}</b></div>
      <div class="stat"><label>可收藏作品</label><b>{{ stats.s1 }}</b></div>
      <div class="stat"><label>已收藏作品</label><b>{{ stats.s2 }}</b></div>
      <div class="stat"><label>再次流通中</label><b>{{ stats.s3 }}</b></div>
    </div>
    <div class="card">
      <div class="filters">
        <div class="field"><label>关键词</label><input v-model="filters.keyword" placeholder="搜索编号 / 作品 / 艺术家" /></div>
        <div class="field"><label>作品状态</label><select v-model="filters.status"><option value="">全部</option><option value="AVAILABLE">可收藏</option><option value="SOLD">已收藏</option><option value="RESALE">再次流通</option></select></div>
    ...
    <div v-if="detailVisible" class="modal" @click.self="detailVisible=false">
      <div class="modal-box">
        <div class="modal-head"><div><h2>作品管理详情</h2><p>{{ current?.title }}</p></div><button class="text" style="color:white;font-size:28px" @click="detailVisible=false">×</button></div>
...
async function refresh(){
  ...
  try {
    ...
    stats.s1 = data.filter(i => i.status === 'on_sale').length
    stats.s2 = data.filter(i => i.status === 'sold').length
    stats.s3 = data.filter(i => i.status === 'resale').length
  } catch (e) {
