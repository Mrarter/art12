<template>
  <div class="page">
    <div class="page-header">
      <div><h1>收藏证书管理</h1><p>管理成交作品的收藏证书生成、预览、下载、寄送与藏家身份展示。</p></div>
      <div class="actions"><button class="ghost" @click="resetFilters">重置筛选</button><button class="primary" @click="refresh">刷新</button></div>
    </div>
    <div class="stats">
      <div class="stat"><label>待生成证书</label><b>{{ stats.s0 }}</b></div>
      <div class="stat"><label>已生成证书</label><b>{{ stats.s1 }}</b></div>
      <div class="stat"><label>待寄送证书</label><b>{{ stats.s2 }}</b></div>
      <div class="stat"><label>今日生成</label><b>{{ stats.s3 }}</b></div>
    </div>
    <div class="card">
      <div class="filters">
        <div class="field"><label>关键词</label><input v-model="filters.keyword" placeholder="搜索编号 / 作品 / 艺术家 / 藏家" /></div>
        <div class="field"><label>证书状态</label><select v-model="filters.status"><option value="">全部</option><option value="PENDING">待处理</option><option value="DONE">已完成</option></select></div>
    ...
    <div v-if="detailVisible" class="modal" @click.self="detailVisible=false">
      <div class="modal-box">
        <div class="modal-head"><div><h2>收藏证书管理详情</h2><p>{{ current?.title }}</p></div><button class="text" style="color:white;font-size:28px" @click="detailVisible=false">×</button></div>
...
import { getCertList, generateCert } from '@/api/certificate'
...
async function refresh(){
  ...
  try {
    ...
    list.value = data.map(item => ({
      id: item.id,
      title: item.cert_no || item.certNo || '',
      sub: (item.artwork_name || item.artworkName || ''),
      statusText: item.status === 'ISSUED' ? '已生成' : item.status === 'PENDING' ? '待生成' : (item.status || ''),
      badgeClass: item.status === 'ISSUED' ? 'green' : item.status === 'PENDING' ? 'red' : 'gray',
      amountText: '',
      extra: item.holder_name || item.holderName || '',
      tags: [item.status === 'ISSUED' ? '已生成' : '待生成'],
      updatedAt: (item.issue_date || item.issueDate || item.create_time || '').slice(0, 16),
      coverUrl: '',
      _raw: item
    }))
    stats.s0 = data.filter(i => !i.status || i.status === 'PENDING').length
    stats.s1 = data.filter(i => i.status === 'ISSUED').length
    stats.s2 = 0
    stats.s3 = 0
  } catch (e) {
