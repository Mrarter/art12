...
const getArtistSpacingKey = (item) => {
  const key = item?.artistUid || item?.artistId || item?.artistName || item?.authorName
  return key ? String(key) : `artwork-${item?.id || Math.random()}`
}

const spreadSimilarArtworks = (list = [], previousList = []) => {
  const shuffledList = shuffleList(list)
  if (shuffledList.length <= 1) return shuffledList

  const groupedMap = new Map()
  shuffledList.forEach((item) => {
    const key = getArtistSpacingKey(item)
    if (!groupedMap.has(key)) groupedMap.set(key, [])
    groupedMap.get(key).push(item)
  })

  const recentKeys = previousList
    .slice(-SIMILAR_ARTWORK_GAP)
    .map(getArtistSpacingKey)
    .filter(Boolean)

  const result = []
  while (result.length < shuffledList.length) {
    const candidates = Array.from(groupedMap.entries())
      .filter(([, items]) => items.length > 0)
      .sort((a, b) => b[1].length - a[1].length)

    if (!candidates.length) break

    let nextGroup = candidates.find(([key]) => !recentKeys.includes(key))
    if (!nextGroup) nextGroup = candidates[0]

    const [nextKey, items] = nextGroup
    const nextItem = items.shift()
    result.push(nextItem)
    recentKeys.push(nextKey)
    if (recentKeys.length > SIMILAR_ARTWORK_GAP) {
      recentKeys.shift()
    }
  }
...
const fetchProductList = async (reset = false) => {
  ...
  try {
    ...
    if (currentTab.value === 'recommend') {
      list = spreadSimilarArtworks(list, reset ? [] : productList.value)
    }
    ...
    try {
      ...
      if (currentTab.value === 'recommend') {
        fallbackList = spreadSimilarArtworks(fallbackList, reset ? [] : productList.value)
      }
...
const onBannerClick = (banner) => {
  ...
  switch (banner.linkType) {
    case 'gallery':
      ...
      break
    case 'auction':
      uni.navigateTo({ url: `/pages/auction-flow/session?id=${banner.linkValue}` })
      break
    case 'artist':
      uni.navigateTo({ url: `/pages/artist/home?userId=${banner.linkValue}` })
      break
    case 'article':
      uni.navigateTo({ url: `/pages/content/article?id=${banner.linkValue}` })
      break
    case 'home':
      uni.switchTab({ url: '/pages/index/index' })
      break
    default:
      console.warn('未处理的 Banner 跳转类型:', banner.linkType, banner)
      break
  }
}
...
// 作品接口和转售接口价格均为元；仅在订单/支付边界转换为分。
const formatPrice = (price) => {
  const amount = Number(price || 0)
  if (amount >= 100000) {
    return `${formatYuanNumber(amount / 10000)}万`
  }
