...
<script>
import { getAuctionSessions, getSessionDetail, createDepositOrder, getDepositStatus } from '@/api/auction'
...
export default {
  ...
  methods: {
    ...
    applySessionData(res) {
      ...
      const rawStatus = Number(this.session.status)
      if (rawStatus >= 1 && rawStatus <= 3) this.session.status = rawStatus - 1
    ...
    goLotDetail(lotId) {
      uni.navigateTo({ url: `/pages/auction-flow/bidding?id=${lotId}` })
    },

    goBidding(lot) {
      uni.navigateTo({
        url: `/pages/auction-flow/bidding?id=${lot.id}&title=${encodeURIComponent(lot.title || lot.artworkName || '')}&currentPrice=${lot.currentPrice || lot.startPrice}&image=${encodeURIComponent(lot.coverImage || lot.image || '')}`
      })
    },
    ...
    async handleDeposit(lot) {
      try {
        ...
        const scene = process.env.UNI_PLATFORM === 'app-plus' ? 'app' : 'h5'
        const payment = await createDepositOrder(this.sessionId, { channel: 'alipay', scene })
        if (payment.paid) {
          this.hasDeposit = true
          uni.showToast({ title: '保证金已缴纳' })
          return
        }
        uni.setStorageSync(`auction_deposit_${this.sessionId}`, payment.payNo)
        if (scene === 'app' && payment.orderInfo) {
          await new Promise((resolve, reject) => uni.requestPayment({
            provider: 'alipay', orderInfo: payment.orderInfo, success: resolve, fail: reject
          }))
        ...
        uni.showToast({ title: e?.message || '发起支付失败', icon: 'none' })
      } finally {
        uni.hideLoading()
      }
    },
    ...
    async refreshDepositStatus() {
      ...
      const payNo = uni.getStorageSync(`auction_deposit_${this.sessionId}`)
      if (!payNo) return
      try {
        const status = await getDepositStatus(payNo)
        this.hasDeposit = !!status?.paid
        if (this.hasDeposit) uni.removeStorageSync(`auction_deposit_${this.sessionId}`)
      } catch (e) {
        console.warn('查询保证金状态失败', e)
      }
    },
